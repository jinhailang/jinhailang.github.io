...
var J_KCWidget = (J_KCWidget||{}).prototype={
	...
	J_Slidy:function(o,$config){
		var s = $config;
		var cli = o.find(s.contentCls).children();
		if (s.width)cli.css({width:s.width+"px"});
		if (s.height)cli.css({height:s.height+"px"});
		this.J_Carousel(o,s);
	},
	J_Carousel:function(o,$config){
		var s = $.extend({
				...
				imgload:false,
				touch:false //是否支持移动端触屏
				},$config || {}),P = {},T = false,$$ = this;

		s.fn = {//全局控制
			unbind:function(){
				clearInterval(T);P.nli.unbind();o.unbind();unloadBtn();
				if (s.circular) {
					var html = $(s.contentCls,o).html();
					$(s.contentCls,o).html(html.split("<!-- ## -->")[0]);
				}
			},
			start:function(){this.unbind();J_KCWidget.init(o,s);},
			to:function(i){myShow(i);},
			next:function(){myShow(s.activeIndex+1);},
			pre:function(){myShow(s.activeIndex-1);}
		};
		init();touch();
		...
		function touch(l){
			if (!s.touch) return ;
			if ('touchwipe' in jQuery.fn){
				$(o).touchwipe({
					 wipeLeft: function() { myShow(s.activeIndex-1); },
					 wipeRight: function() {myShow(s.activeIndex+1); },
					 wipeUp: function() { myShow(s.activeIndex-1);},
					 wipeDown: function() {myShow(s.activeIndex+1); },
					 min_move_x: 20,
					 min_move_y: 20,
					 preventDefaultEvents: true
				});
			}else{
				if (!l)$.getScript(J_KCWidget.Path +J_KCWidget.b.touch,touch(true));
			}
		}
		function init(){
			P = {c : $(s.contentCls,o),n : $(s.navCls,o),hr : "<hr style='display:none;' />"};
			P.c.html(one(P.c.html()));
			P.cli = P.c.children();
			P.len = P.cli.length;
			P.page = Math.ceil(P.len/s.steps);
			P.u = (s.viewSize) ? s.viewSize : ( (s.effect=="scrollx") ? P.cli.eq(0).width() : P.cli.eq(0).height() )*s.steps;//一屏尺寸
			P.px  = P.u*P.page;//总尺寸
			P.s = s.duration*1000;
			P.k = false;
			loadBtn();
			if (!P.len<s.steps)deploy();
		}
		function one(cStr){
			var h = ((cStr).split(P.hr))[0];
		}
		function deploy(){//初始化节点对象
			P.n = (!P.n[0]) ? $("<ul></ul>").appendTo(o).addClass(s.navCls.substr(1)) : P.n;//避免navCls 不存在的情况
			P.nli = rPn(P.n.children());
			if (s.effect=="fade"||s.effect=="none") P.cli.hide().css({position: "absolute",top:"0",left:"0"});//
			else{
				P.cli.css({float:"left",display:"inline",overflow:"hidden"});
				if (s.circular) P.c.append(P.hr+P.c.html());
				if (s.effect=="scrollx")P.c.css({width:"99999px"});
				if (s.effect=="scrolly")P.cli.css({clear:"left"});
			}
			if (s.imgload){
				P.c.J_KCWidget_imgload('reset',s.placeholder);
			}
			if (s.interval*1000>100) myShow(s.activeIndex,true);//第一次播放
			else setforli(s.activeIndex-1);
			s.triggerType = (s.triggerType=='click')?'click':'mouseover';
			P.nli.bind(s.triggerType, function(event){myShow(P.nli.index(this)+1,true);});
		}
		function rPn(li){
			if (typeof(li[0])!='object'||P.n.html()==''){
				for (var i=0;i<P.page ;i++ ){
					P.n.append("<li>"+(i+1)+"</li>");
				}
			}
			return P.n.children();
		}
		function myShow(i,f){P.k = true;
			var modfi = (i-1)%P.page;modfi = (modfi<0)?modfi+P.page:modfi;//真实索引
			var css={},to = 'margin-left';
			switch(s.effect) {
				case "scrolly": to = "margin-top";
				case "scrollx":
					css[to] = -animate(to,modfi,i)+"px";
					P.c.stop(true,false).animate(css, P.s,function(){
						P.k = false;
					});
					break;
			   case "fade":
					P.cli.eq(modfi).css("z-index",2).fadeIn(P.s,function(){
					}).siblings().stop(true,true).css("z-index",1).fadeOut();P.k = false;break;
			   case "none":
					P.cli.hide().eq(modfi).show();P.k = false;break ;
			   default:
			}
			setforli(modfi);
		...
		*/
		function animate(type,t_m,i){
			var re = P.u*((i<1||(!s.circular))? t_m :(i-1));
			var t_n = parseInt(P.c.css(type));
			var css={};
			if (s.circular){
				if (Math.abs(t_n)>=P.px){
					css[type] = (t_n%P.px)+"px";//默认回到原像原位置
				}
				if (i<1){
					css[type] = -(Math.abs(t_n%P.px)+P.px)+ "px";//向上翻过0点，重置到镜像
				}
				if (css[type]) P.c.css(css);//动作前初始化当前位置
			}
			return re;
		}
		...
		function setdisplay(){//设置前后按钮样式
			if (!s.circular&&s.disableBtnCls!=""){
				...
				if (s.activeIndex == P.page) $(s.nextBtnCls,o).addClass(s.disableBtnCls);
		...
		function play(){
			...
			if (s.interval*1000<100&&(s.effect=='scrollx'||s.effect=='scrolly')){
			  ...
			  doing = function(){
				 var d = Math.abs(parseInt(P.c.css(type)));
				 d = (d == P.px)? 0 : (d+1);
				 css[type] = -d+"px";
				 P.c.css(css);
				 if (d%P.u==0) setforli(d/P.u);
			  }
			}else{
				doing = function(){
		...
		function setforli(i){
			P.nli.eq(i).addClass(s.activeTriggerCls).siblings().removeClass(s.activeTriggerCls);

			if (s.imgload){
				for (var j=0;j<((i+2)>P.page ? P.page : (i+2))*s.steps;j++){
					P.c.children().eq(j).add(P.c.children().eq(j+P.len)).J_KCWidget_imgload('load',s.placeholder);
				}
			}
			s.activeIndex = (i+1);
		}
		function loadBtn(){
			if (s.prevBtnCls!=""){
				o.find(s.prevBtnCls).bind("click",function(){if (!P.k) myShow(s.activeIndex-1);});
			}
			if (s.nextBtnCls!=""){
				o.find(s.nextBtnCls).bind("click",function(){if (!P.k) myShow(s.activeIndex+1);});
			}
		}
		function unloadBtn(){
...
(function($){
	$.fn.J_KCWidget = function() {
		J_KCWidget.b = {
			me :"jquery.J_KCWidget.js",
			touch :"jquery.touchy.js"
			};
		J_KCWidget.Path = J_KCWidget.Path || J_KCWidget.f.getScriptPath(J_KCWidget.b.me);
		this.each(function(){
